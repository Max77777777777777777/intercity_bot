...
def _init_pool():
    ...
    log.info("✅ Пул соединений PostgreSQL создан (2–20)")
...
class DB:
    ...
    def init():
        with DB.conn() as c:
            ...
            """)
            cur.execute("CREATE TABLE IF NOT EXISTS subscriptions(user_id BIGINT PRIMARY KEY, expires_date TEXT)")
            cur.execute("""
            ...
            cur.execute("CREATE TABLE IF NOT EXISTS pending_subscriptions(user_id BIGINT PRIMARY KEY, plan_key TEXT)")
            cur.execute("""
            ...
            """)
            cur.execute("CREATE INDEX IF NOT EXISTS idx_orders_passenger ON orders(passenger_id)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_orders_driver    ON orders(driver_id)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_orders_status    ON orders(status)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_ratings_driver   ON ratings(driver_id)")
            cur.execute("CREATE INDEX IF NOT EXISTS idx_ratings_order    ON ratings(order_id)")
        log.info("✅ БД готова (PostgreSQL)")

    @staticmethod
    def driver(uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT * FROM drivers WHERE user_id=%s", (uid,))
            r = cur.fetchone()
    ...
    def driver_save(uid, d):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute(
                "INSERT INTO drivers VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) "
    ...
    def driver_update_fields(uid, **fields):
        ...
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute(f"UPDATE drivers SET {sets} WHERE user_id=%s", list(fields.values()) + [uid])

    @staticmethod
    def driver_del(uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT id, passenger_id FROM orders WHERE driver_id=%s AND status='taken'", (uid,))
            active_orders = [(r["id"], r["passenger_id"]) for r in cur.fetchall()]
            cur.execute("UPDATE ratings SET driver_id=NULL WHERE driver_id=%s", (uid,))
            for tbl in ("drivers","subscriptions","pending_subscriptions"):
                cur.execute(f"DELETE FROM {tbl} WHERE user_id=%s", (uid,))
            cur.execute("UPDATE orders SET status='open',driver_id=NULL,taken_at=NULL WHERE driver_id=%s AND status='taken'", (uid,))
    ...
    def driver_verify(uid, v):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("UPDATE drivers SET docs_verified=%s WHERE user_id=%s", (1 if v else 0, uid))
    ...
    def active_drivers():
        ...
        with DB.conn() as c:
            ...
            cur.execute("""
                ...
                WHERE d.docs_verified=1 AND s.expires_date>=%s
    ...
    def sub_info(uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT expires_date FROM subscriptions WHERE user_id=%s", (uid,))
            r = cur.fetchone()
    ...
    def sub_set(uid, exp, admin_id=None, plan_key=None):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("INSERT INTO subscriptions VALUES (%s,%s) ON CONFLICT (user_id) DO UPDATE SET expires_date=EXCLUDED.expires_date", (uid, exp))
            if admin_id and plan_key:
                cur.execute("INSERT INTO subscription_log (user_id,plan_key,admin_id,action,created_at) VALUES (%s,%s,%s,%s,%s)", (uid, plan_key, admin_id, "activate", now_iso()))
    ...
    def order_upd(oid, **kw):
        ...
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute(f"UPDATE orders SET {sets} WHERE id=%s", list(kw.values()) + [oid])
    ...
    def order_cancel_atomic(oid, uid, role="passenger"):
        with DB.conn() as c:
            cur = c.cursor()
            if role == "passenger":
                cur.execute("UPDATE orders SET status='cancelled' WHERE id=%s AND passenger_id=%s AND status IN ('open','taken','pending')", (oid, uid))
            else:
                cur.execute("UPDATE orders SET status='open',driver_id=NULL,taken_at=NULL WHERE id=%s AND driver_id=%s AND status='taken'", (oid, uid))
    ...
    def order_take_atomic(oid, uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("UPDATE orders SET status='taken',driver_id=%s,taken_at=%s WHERE id=%s AND status='open'", (uid, now_iso(), oid))
            return cur.rowcount > 0
    ...
    def passenger_orders(uid, limit=5):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT * FROM orders WHERE passenger_id=%s ORDER BY created_at DESC LIMIT %s", (uid, limit))
            return [dict(r) for r in cur.fetchall()]
    ...
    def driver_orders(uid, limit=5):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT * FROM orders WHERE driver_id=%s ORDER BY taken_at DESC LIMIT %s", (uid, limit))
            return [dict(r) for r in cur.fetchall()]
    ...
    def open_orders():
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT * FROM orders WHERE status='open' ORDER BY created_at DESC")
            return [dict(r) for r in cur.fetchall()]
    ...
    def all_orders(limit=10, offset=0, status=None):
        with DB.conn() as c:
            cur = c.cursor()
            if status:
                cur.execute("SELECT * FROM orders WHERE status=%s ORDER BY created_at DESC LIMIT %s OFFSET %s", (status, limit, offset))
                rows = cur.fetchall()
    ...
    def stats():
        ...
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT COUNT(*) as cnt FROM orders"); t = cur.fetchone()["cnt"]
            cur.execute("SELECT COUNT(*) as cnt FROM orders WHERE status='open'"); o = cur.fetchone()["cnt"]
            cur.execute("SELECT COUNT(*) as cnt FROM orders WHERE status='completed'"); d = cur.fetchone()["cnt"]
            cur.execute("SELECT COUNT(*) as cnt FROM drivers"); dr = cur.fetchone()["cnt"]
            cur.execute("SELECT COUNT(*) as cnt FROM drivers WHERE docs_verified=1"); dc = cur.fetchone()["cnt"]
            cur.execute("SELECT COUNT(*) as cnt FROM drivers d JOIN subscriptions s ON s.user_id=d.user_id WHERE s.expires_date>=%s", (today,))
            sub = cur.fetchone()["cnt"]
    ...
    def bl_add(uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("INSERT INTO blacklist VALUES (%s) ON CONFLICT DO NOTHING", (uid,))
    ...
    def pending_set(uid, pk):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("INSERT INTO pending_subscriptions VALUES (%s,%s) ON CONFLICT (user_id) DO UPDATE SET plan_key=EXCLUDED.plan_key", (uid, pk))

    @staticmethod
    def pending_get(uid):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT plan_key FROM pending_subscriptions WHERE user_id=%s", (uid,))
            r = cur.fetchone()
    ...
    def pending_del(uid, admin_id=None, plan_key=None):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("DELETE FROM pending_subscriptions WHERE user_id=%s", (uid,))
            if admin_id and plan_key:
                cur.execute("INSERT INTO subscription_log (user_id,plan_key,admin_id,action,created_at) VALUES (%s,%s,%s,%s,%s)", (uid, plan_key, admin_id, "reject", now_iso()))

    @staticmethod
    def pending_all():
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT * FROM pending_subscriptions")
            return [dict(r) for r in cur.fetchall()]
    ...
    def add_rating(order_id, driver_id, passenger_id, stars):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT id FROM ratings WHERE order_id=%s AND passenger_id=%s", (order_id, passenger_id))
            existing = cur.fetchone()
            if existing:
                cur.execute("UPDATE ratings SET stars=%s,driver_id=%s,created_at=%s WHERE order_id=%s AND passenger_id=%s", (stars, driver_id, now_iso(), order_id, passenger_id))
            else:
                cur.execute("INSERT INTO ratings (order_id,driver_id,passenger_id,stars,created_at) VALUES (%s,%s,%s,%s,%s)", (order_id, driver_id, passenger_id, stars, now_iso()))

    @staticmethod
    def avg_rating(driver_id):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT AVG(stars) as avg, COUNT(*) as cnt FROM ratings WHERE driver_id=%s", (driver_id,))
            r = cur.fetchone()
    ...
    def has_rating(order_id, passenger_id):
        with DB.conn() as c:
            cur = c.cursor()
            cur.execute("SELECT 1 FROM ratings WHERE order_id=%s AND passenger_id=%s", (order_id, passenger_id))
            return cur.fetchone() is not None
...
async def _notified_cleaner():
    while True:
        try:
            ...
            try:
                placeholders = ",".join(["%s"] * len(current_oids))
                with DB.conn() as c:
                    cur = c.cursor()
                    cur.execute(
