...
class DB:
    ...
    async def stats():
        today = now_dt().date().strftime("%Y-%m-%d")
        async with _pg_pool.acquire() as c:
            t   = await c.fetchval("SELECT COUNT(*) FROM orders")
            o   = await c.fetchval("SELECT COUNT(*) FROM orders WHERE status='open'")
            d   = await c.fetchval("SELECT COUNT(*) FROM orders WHERE status='completed'")
            dr  = await c.fetchval("SELECT COUNT(*) FROM drivers")
            dc  = await c.fetchval("SELECT COUNT(*) FROM drivers WHERE docs_verified=1")
            sub = await c.fetchval(
                "SELECT COUNT(*) FROM drivers d JOIN subscriptions s ON s.user_id=d.user_id WHERE s.expires_date>=$1", today)
...
class BlacklistMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        ...
        if uid and await DB.bl_check(uid):
...
async def cb_sub(call: CallbackQuery):
    uid = call.from_user.id
    if not await DB.driver(uid): await call.answer("❌ Заполните профиль", show_alert=True); return
...
async def _send_drivers_page(uid, page=0):
    ...
    for d in chunk:
        exp_str, dl, active = await DB.sub_info(d["user_id"])
        avg, cnt = await DB.avg_rating(d["user_id"])
