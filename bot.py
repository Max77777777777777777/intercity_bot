# ══════════════════════════════════════════════════════════════
#  МЕЖГОРОД ТРАНСФЕР v13.1 — aiogram 3
# ══════════════════════════════════════════════════════════════
import asyncio, sqlite3, json, re, html, logging, os, time
import urllib3
...
class DB:
    ...
    def session(uid):
        ...
        return ({"role": r["role"], "step": r["step"], "data": json.loads(r["data"] or "{}")}
    ...
    def driver_update_fields(uid, **fields):
        ...
        for k, v in fields.items():
            if v is None:
                raise ValueError(f"Поле {k} не может быть None")
    ...
    def driver_del(uid):
        ...
        log.info(f"driver_del: водитель {uid} удалён, активных заказов: {len(active_orders)}")
        return active_orders  # caller sends notifications async
...
async def _notified_cleaner():
    while True:
        ...
        try:
            ...
            with DB.conn() as c:
                rows = c.execute(
                    f"SELECT id, status FROM orders WHERE id IN ({placeholders})", current_oids
                ).fetchall()
        ...
        if dead:
            async with _NLOCK:
                for oid in dead:
                    _NOTIFIED.pop(oid, None)
            log.info(f"🧹 Кэш уведомлений: очищено {len(dead)} заказов")
...
async def update_channel_post(oid):
    ...
    if order["status"] in ("cancelled","taken"):
        try:
            await bot.delete_message(GROUP_CHAT_ID, order["channel_msg_id"])
        except Exception as e:
...
async def to_main(msg: Message):
    DB.session_upd(msg.from_user.id, role="", step="")
...
async def order_start(msg: Message):
    ...
    if s.get("step") in ["from_city","to_city","trip_date","trip_time","passengers","car_class","wishes"]:
        await safe_send(uid, "⚠️ У вас уже есть активное создание заказа. Завершите его или нажмите «❌ Отменить»",
...
async def step_to(msg: Message):
    uid, city = msg.from_user.id, msg.text.strip()
    if not is_valid_city(city): await safe_send(uid, "❌ Некорректное название"); return
    s = DB.session(uid)
    from_city = s["data"].get("from_city")
    if not from_city:
        DB.session_clr(uid)
        await safe_send(uid, "⚠️ Данные сессии устарели. Начните создание заказа заново.", reply_markup=kb_main())
        return
    if from_city.lower() == city.lower():
        await safe_send(uid, "❌ Города отправления и назначения не должны совпадать"); return
    s["data"]["to_city"] = city
    DB.session_upd(uid, step="trip_date", data=s["data"])
    await safe_send(uid, f"✅ Куда: <b>{esc(city)}</b>\n\n🗺 <b>Шаг 3/7</b>\n📅 Дата (ДД.ММ.ГГГГ)")

async def step_date(msg: Message):
    uid = msg.from_user.id
    try:
...
async def step_time(msg: Message):
    uid, t = msg.from_user.id, msg.text.strip()
    if not re.match(r"^\d{1,2}:\d{2}$", t):
        await safe_send(uid, "❌ Формат ЧЧ:ММ"); return
    try:
        h, m = map(int, t.split(":"))
        if not (0 <= h <= 23 and 0 <= m <= 59): await safe_send(uid, "❌ Некорректное время"); return
        s = DB.session(uid)
        if ds := s["data"].get("trip_date"):
            trip_dt = datetime.strptime(f"{ds} {t}", "%d.%m.%Y %H:%M").replace(tzinfo=TZ)
            if trip_dt < now_dt(): await safe_send(uid, "❌ Поездка не может быть в прошлом"); return
...
async def step_wishes(msg: Message):
    uid, wish = msg.from_user.id, msg.text.strip()
    if len(wish) > 500: await safe_send(uid, "❌ Слишком длинный текст (макс. 500 символов)"); return
    s = DB.session(uid)
    s["data"]["wishes"] = "" if wish.lower() in ["нет","—","-","no"] else wish
    DB.session_upd(uid, data=s["data"])
    await finalize_order(uid)

async def finalize_order(uid):
    s = DB.session(uid); data = s["data"]
    DB.session_clr(uid)
    await safe_send(uid, "⏳ Рассчитываю...")
    asyncio.create_task(_finalize_order_task(uid, data))
...
@check_blacklist
async def driver_enter(msg: Message):
    uid = msg.from_user.id
    DB.session_upd(uid, role="driver")
    drv = DB.driver(uid)
    if not drv:
...
async def register_driver(msg: Message):
    uid = msg.from_user.id
    if DB.driver(uid): await safe_send(uid, "ℹ️ Вы уже зарегистрированы.", reply_markup=kb_driver(uid)); return
    await _start_profile(uid)
...
async def p_contact(msg: Message):
    uid  = msg.from_user.id
...
async def p_model(msg: Message):
    uid, model = msg.from_user.id, msg.text.strip()
    if not (2 <= len(model) <= 100): await safe_send(uid, "❌ Введите марку и модель (2–100 символов)"); return
...
async def p_year(msg: Message):
    ...
    try:
        ...
        log.error(f"p_year DB error uid={uid}: {e}")
        await safe_send(uid, "❌ Ошибка сохранения, попробуйте ещё раз")

async def p_number(msg: Message):
    uid, number = msg.from_user.id, msg.text.strip().upper().replace(" ","")
    if not number: await safe_send(uid, "❌ Введите номер автомобиля"); return
...
async def _finalize_profile(uid):
    s    = DB.session(uid)
    data = s["data"]
...
# ── Выбор пассажиров ──
@router.callback_query(F.data.startswith("pax_"))
async def cb_pax(call: CallbackQuery):
    uid = call.from_user.id
    s = DB.session(uid)
    if s.get("step") != "passengers": await call.answer("❌"); return
    s["data"]["passengers"] = int(call.data.split("_")[1])
    DB.session_upd(uid, data=s["data"])
    await safe_edit_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    await safe_send(uid, f"✅ Пассажиров: <b>{s['data']['passengers']}</b>")
    await show_car_class(uid)
    await call.answer()
...
async def cb_car(call: CallbackQuery):
    uid = call.from_user.id
    s = DB.session(uid)
    if s.get("step") != "car_class": await call.answer("❌"); return
    ...
    await ask_wishes(uid)
    await call.answer()

# ── Класс авто при регистрации водителя ──
@router.callback_query(F.data.startswith("pclass_"))
async def cb_pclass(call: CallbackQuery):
    uid = call.from_user.id
    s   = DB.session(uid)
    if s.get("step") != "profile_car_class":
        await call.answer("❌ Неверный шаг"); return
    cc = call.data.split("_", 1)[1]
...
async def cb_cancel_order(call: CallbackQuery):
    ...
    DB.session_clr(uid)
    await safe_edit_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    await safe_send(uid, "❌ Отменено.", reply_markup=kb_driver(uid) if DB.driver(uid) else kb_main())
    await call.answer()
...
async def cb_rate(call: CallbackQuery):
    ...
    if len(parts) != 3: await call.answer("❌"); return
...
async def cb_take(call: CallbackQuery):
    ...
    od = DB.order(oid)
    if not od:
        log.error(f"cb_take: заказ #{oid} исчез после order_take_atomic, uid={uid}")
        await call.answer("✅ Заказ принят!"); return
    pid = od.get("passenger_id")
    if not pid:
        log.error(f"cb_take: passenger_id=None у заказа #{oid}")
        await call.answer("✅ Заказ принят!"); return
    try:
...
async def cb_adm_ord_page(call: CallbackQuery):
    ...
    try:
        ...
        page       = int(parts[-1])
        filter_key = parts[-2]
        if filter_key not in ORDER_FILTERS: filter_key = "all"
...
async def cb_adm_ord_noop(call: CallbackQuery):
    await call.answer()

# ── Отмена/завершение заказа (админ) ──
...
async def cb_adm_drv_noop(call: CallbackQuery):
    await call.answer()
...
async def cb_adm_ef(call: CallbackQuery):
    uid = call.from_user.id
    if uid not in ADMIN_IDS: return
...
STEP_REQUIRED_DATA = {
    "to_city":    ["from_city"],
    "trip_date":  ["from_city", "to_city"],
    "trip_time":  ["from_city", "to_city", "trip_date"],
    "passengers": ["from_city", "to_city", "trip_date", "trip_time"],
    "car_class":  ["from_city", "to_city", "trip_date", "trip_time", "passengers"],
    "wishes":     ["from_city", "to_city", "trip_date", "trip_time", "passengers", "car_class"],
}
STEP_HANDLERS = {
    "from_city":          step_from,
    "to_city":            step_to,
...
@router.message()
async def universal_text(msg: Message):
    uid  = msg.from_user.id
    text = (msg.text or "").strip()

    if DB.bl_check(uid):
        await safe_send(uid, "⛔ Вы заблокированы."); return

    if text == "❌ Отменить":
        await order_cancel(msg); return
    ...
    if step and step.startswith("adm_ef_") and uid in ADMIN_IDS:
        ...
        drv = DB.driver(tgt)
        if not drv:
            await safe_send(uid, "❌ Водитель не найден"); DB.session_clr(uid); return
        ...
        if field == "car_model":
            ...
            DB.session_clr(uid)
            await safe_send(uid, f"✅ Марка/модель обновлены: {esc(text)}", reply_markup=kb_main())
            ...
            try:
                ...
                DB.session_clr(uid)
                await safe_send(uid, f"✅ Год выпуска обновлён: {year}", reply_markup=kb_main())
                await notify_driver_change(tgt, "год выпуска авто", year)
            except ValueError:
                await safe_send(uid, f"❌ Некорректный год (минимум {MIN_CAR_YEAR})")
            ...
            DB.session_clr(uid)
            await safe_send(uid, f"✅ Гос. номер обновлён: {esc(number)}", reply_markup=kb_main())
            await notify_driver_change(tgt, "гос. номер", number)
        return

    if step in STEP_HANDLERS:
        required = STEP_REQUIRED_DATA.get(step, [])
        missing  = [k for k in required if k not in s["data"]]
        if missing:
            log.warning(f"universal_text: шаг={step}, нет данных={missing}, uid={uid}")
            DB.session_clr(uid)
            await safe_send(uid,
                "⚠️ Данные сессии устарели. Начните создание заказа заново.",
                reply_markup=kb_main())
            return
        await STEP_HANDLERS[step](msg)
    elif step:
        await safe_send(uid, "⚠️ Введите данные или нажмите «❌ Отменить»")
    else:
        await safe_send(uid, "👇 Используйте кнопки меню", reply_markup=kb_main())


# ══════════════ ЗАПУСК ══════════════
async def main():
    log.info("=" * 50)
    log.info("  🚕 МЕЖГОРОД ТРАНСФЕР v13.1 (aiogram 3)")
    log.info("=" * 50)
    DB.init()
    asyncio.create_task(_notified_cleaner())
