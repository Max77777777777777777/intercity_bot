# ══════════════════════════════════════════════════════════════
#  МЕЖГОРОД ТРАНСФЕР v16.1 — aiogram 3 + FSM
#  new: тариф Кавказ/СНГ, OSRM, рейсы, поиск места
#  fix: trips_search фильтрует прошедшие рейсы текущего дня
#  fix: trip_book_atomic — явная проверка дубля в транзакции
#  fix: валидация цены рейса 100–50000 ₽
# ══════════════════════════════════════════════════════════════
...
TARIFFS_RF = {
    ...
    "minivan":  {"label": "🚐 Минивэн / Компактвэн", "year": "",           "price": 45,   "price_seat": 10},
...
TARIFFS_NT = {
    ...
    "minivan":  {"label": "🚐 Минивэн / Компактвэн", "year": "",           "price": 65,   "price_seat": 14},
...
TARIFFS_CIS = {
    ...
    "minivan":  {"label": "🚐 Минивэн / Компактвэн", "year": "",           "price": 55,   "price_seat": 15.5},
...
def fmt_trip(t, drv=None, avg=None, cnt=None):
    ...
    lines = [
        ...
        f"📅 {esc(t.get('trip_date'))} · 🕐 {esc(t.get('trip_time'))}",
        f"📏 {dist_str}",
...
async def cmd_deldrv(msg: Message):
    ...
    try:
        ...
        active_orders = await DB.driver_del(t)
...
async def cb_conf_sub(call: CallbackQuery):
    ...
    await DB.pending_del(tgt, uid, pk)
    await safe_edit_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
...
async def cb_adm_delok(call: CallbackQuery):
    ...
    drv = await DB.driver(tgt)
    if drv:
        active_orders = await DB.driver_del(tgt)
...
async def unknown_state_after_restart(msg: Message, state: FSMContext):
    cur = await state.get_state()
    if cur is not None:
        # Пользователь в активном шаге, который ждёт нажатия инлайн-кнопки,
        # а не текстового сообщения (например, выбор класса авто/числа пассажиров).
        await safe_send(msg.from_user.id, "ℹ️ Пожалуйста, используйте кнопки выше, чтобы продолжить.")
        return
    known = {
...
async def trip_start(msg: Message, state: FSMContext):
    ...
    drv = await DB.driver(uid)
    if not drv or not drv.get("docs_verified"):
        await safe_send(uid, "❌ Профиль не верифицирован."); return
    _, _, active = await DB.sub_info(uid)
    if not active:
        await safe_send(uid, "❌ Нет активного абонемента.", reply_markup=kb_subs()); return
...
async def trip_from(msg: Message, state: FSMContext):
    uid, city = msg.from_user.id, msg.text.strip()
    if city == "❌ Отменить":
        await state.clear(); await safe_send(uid, "❌ Отменено.", reply_markup=await kb_driver(uid)); return
    if not is_valid_city(city):
        await safe_send(uid, "❌ Введите корректное название города."); return
    await state.update_data(from_city=city)
...
async def trip_to(msg: Message, state: FSMContext):
    ...
    if city.lower() == data.get("from_city","").lower():
        await safe_send(uid, "❌ Откуда и куда совпадают."); return
    await state.update_data(to_city=city)
...
async def trip_date_step(msg: Message, state: FSMContext):
    ...
    try:
        ...
        await safe_send(uid, "❌ Неверный формат. Введите ДД.ММ.ГГГГ."); return
    await state.update_data(trip_date=iso)
    await state.set_state(TripForm.trip_time)
    await safe_send(uid, f"✅ Дата: <b>{text}</b>\n\n🕐 <b>Шаг 4/6</b> — Время?\n<i>Формат: ЧЧ:ММ</i>")
...
async def trip_time_step(msg: Message, state: FSMContext):
    ...
    if not re.match(r"^\d{1,2}:\d{2}$", text):
        await safe_send(uid, "❌ Неверный формат. Введите ЧЧ:ММ."); return
    try:
...
async def _trip_show_confirm(uid: int, state: FSMContext):
    ...
    await safe_send(uid,
        ...
        f"📅 {data.get('trip_date')} · 🕐 {data.get('trip_time')}\n"
        f"📏 {dist_str}\n"
        f"🚘 {esc(data.get('car_class_label'))}\n"
        f"💺 Мест: {seats}\n"
...
async def my_trips(msg: Message):
    uid = msg.from_user.id
    if not await DB.driver(uid):
...
async def cb_trip_drv_cancel(call: CallbackQuery):
    ...
    for pid in passengers:
        await safe_send(pid,
            ...
            f"📍 {esc(trip['from_city'])} → {esc(trip['to_city'])} · {trip['trip_date']}")
...
@router.message(F.text == "🚐 Поехать вместе")
...
async def search_from(msg: Message, state: FSMContext):
    uid, city = msg.from_user.id, msg.text.strip()
    if city == "❌ Отменить":
        await state.clear(); await safe_send(uid, "❌ Поиск отменён.", reply_markup=kb_main()); return
    if not is_valid_city(city):
        await safe_send(uid, "❌ Введите корректное название города."); return
    await state.update_data(from_city=city)
...
async def search_date(msg: Message, state: FSMContext):
    ...
    if not trips:
        await safe_send(uid,
            f"😔 Рейсов <b>{esc(from_city)} → {esc(to_city)}</b> на <b>{text}</b> не найдено.\n\n"
            "Попробуйте другую дату или создайте обычный заказ.",
            reply_markup=kb_main())
        return
    await safe_send(uid,
        f"✅ Найдено рейсов: <b>{len(trips)}</b>\n📍 {esc(from_city)} → {esc(to_city)} · {text}")
    for t in trips:
